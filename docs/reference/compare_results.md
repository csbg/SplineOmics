...
- splineomics1:
  ...
  `SplineOmics`: A list containing `limma_splines_result` with topTables
  organized by category and subcategory.

- splineomics2:

  `SplineOmics`: Same structure as `splineomics1`; the object to
  compare.
...
- adj_p_tresh1:
  ...
  `numeric(1)`: Numeric p-value threshold for calling hits in analysis
  1.
...
- verbose:
  ...
  `Boolean(1)`: Boolean flag indicating whether info messages should be
  shown.
