...
over time after the CHO cells were fed (= impact of the feeding). Further, 
the hits (proteins with a significant change over time) are clustered based on
their temporal pattern, and a gene set enrichment analysis is performed with 
each cluster to see if there are processes which are up- or downregulated
...
```{r Define info that is written all the HTML reports}
report_info <- list(
  omics_data_type = "PTX",
