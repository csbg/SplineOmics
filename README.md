...
- **Automatically perform exploratory data analysis:**
  ...
  The `explore_data()` function generates an HTML report with plots
  such as density, PCA, and correlation heatmaps ([example
...
- **Perform limma spline analysis (with optional automatic tuning):**
  ...
  Use `run_limma_splines()` for spline-based `limma` testing. Set
  `spline_params$dof = 0` to select spline degrees of freedom by
  leave-one-out cross-validation; for mixed models, set
  `dream_params$dof = 0` similarly. Use `create_limma_report()` for
  the HTML summary ([example
...
- **Detect local peaks, valleys, and cliffs (PVC analysis):**
  ...
  `find_pvc()` runs the statistical excursion test; `create_pvc_report()`
...
- **Cluster significant features:**
  ...
  `cluster_hits()` clusters hits by spline shape; `create_clustering_report()`
...
- **Run over-representation analysis (ORA) on clustered hits:**
  ...
  `run_ora()` performs clusterProfiler `enricher`-style ORA on gene
  sets per cluster; `create_ora_report()` produces the HTML report
  ([example
  report](https://csbg.github.io/SplineOmics_html_reports/run_ora_report.html)).
  Download gene-set libraries with `download_enrichr_databases()` or
  `extract_gene_sets()` from Bioconductor organism databases.

- **Compare two SplineOmics analyses:**

  `compare_results()` correlates and contrasts hit sets and adjusted
  p-values between two result objects.

- **Cluster genes across multiple omics modalities:**

  `cluster_genes_multiomics()` integrates gene-centric multi-omics
  trajectories; visualize with `plot_umap_clusters()` (see the
  [multi-omics
