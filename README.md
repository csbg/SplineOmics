...
If you have -omics data over time, the package will help you to run `limma` with splines, decide on which parameters to use, perform the clustering, run GSEA and show result plots in HTML reports. Any time-series data that is a valid input to the `limma` package is also a valid input to the `SplineOmics` package (such as transcriptomics, proteomics, phosphoproteomics, metabolomics, etc.).

### What do I need precisely?

1.  A data matrix where each row is a feature (e.g., protein, metabolite, etc.) and each column is a sample taken at a specific time.

2.  A table with identifiers on the rows/features of the data matrix (e.g., gene and protein name)

3.  A table with metadata on the columns/samples of the data matrix (e.g., batch, time point, etc.)
