...
```{r prepare arguments for ORA, eval = TRUE}
...
```{r download BioConductor mouse database, eval = TRUE}
...
```{r prepare arguments for ORA with BioConductor databases, eval = TRUE}
# Specify the filepath of the TSV file with the database info
...
```{r run ORA with BioConductor databases, eval = TRUE}
result <- SplineOmics::run_ora(
  cluster_table = clustering_results[["cluster_table"]],
  databases = databases,
  clusterProfiler_params = clusterProfiler_params,
