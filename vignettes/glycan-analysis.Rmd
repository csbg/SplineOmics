...
run_full_vignette <- identical(Sys.getenv("RUN_FULL_VIGNETTES"), "true")
```
...
```{r limma spline analysis, eval = run_full_vignette}
...
`eBayes`. See the [FAQ](faq.html) and `create_splineomics()` for details.
...
```{r build limma report, eval = run_full_vignette}
...
```{r prepare arguments, eval = run_full_vignette}
nr_clusters <- list(
    constant = 2,
    tshifted = 2
)
...
```{r cluster the hits, eval = run_full_vignette}
...
```{r generate clustering report, eval = run_full_vignette}
