...
-   **Identify proteins with significant temporal changes**: Among the
    ...
    which proteins show a significant change over time after the CHO
    cells were fed (i.e., the impact of the feeding).
...
-   **Perform over-representation analysis (ORA)**: For each cluster,
    ORA will be performed to determine if specific biological processes
    are over-represented among the clustered hits after feeding.
...
The file proteomics_meta.csv contains the meta information, which are the
...
-   The data must be a numeric matrix without non-numeric values.
    Partial `NA` values are tolerated by `limma`, but rows or columns
    that are entirely `NA` are rejected. For reliable results, imputation
    or filtering is still recommended — for example, the original
    proteomics data contained some `NA` values, which were imputed before
    this analysis.
...
-   **Assign rowheaders**:
    ...
    -   If annotation columns are specified (like `"Gene_name"` in this
        example), these values are used to construct the rowheaders
        (feature names). Optionally, the row index can also be included
        to guarantee uniqueness of the feature names.
...
The **SplineOmics** package provides `explore_data()` to run EDA from a
**SplineOmics** object (created with `create_splineomics()`; see below).
The object must already contain `data`, `meta`, `condition`, and
`report_info`. Optional batch columns (`meta_batch_column`,
`meta_batch2_column`) are also read from the object.

**Function arguments:**

-   **`splineomics`**: The SplineOmics object with the inputs above.
-   **`report_dir`**: Directory where HTML reports are saved (defaults
    to `tempdir()`).
-   **`report`**: Logical; set to `FALSE` to return plots only and skip
    the HTML report.

If at least one batch column was set in `create_splineomics()`, the
function will:

-   Use `limma::removeBatchEffect()` before plotting.
...
results back into the object via `update_splineomics()`. You do not need
to manage those updates manually beyond calling the workflow functions.
...
-   **condition**: Meta column name of the levels (e.g., Exponential and
    ...
    conditions are modeled jointly, and SplineOmics automatically
    performs all pairwise comparisons. For each pair of conditions, it
    computes both the average difference (limma Category 2) and the
    condition–time interaction (limma Category 3), providing a complete
    set of contrast results regardless of how many conditions are
    included.
...
Setting `spline_params$dof = 0` lets `run_limma_splines()` choose the
spline degrees of freedom by leave-one-out cross-validation instead of
using a fixed value.
...
The output of `run_limma_splines()` is stored inside the SplineOmics
object and contains a named list where each element represents one of
the limma result categories. Refer to [this
...
on the category, these tables correspond either to individual
conditions or to pairwise comparisons between conditions.
...
condition–time interaction). Optionally, `cluster_hits()` can apply
additional effect-size cutoffs (`min_effect_size`) so that statistically
significant but biologically trivial changes are excluded.

Clustering is performed with k-means on the predicted spline trajectories.
Optionally, clusters can be pruned using `min_cluster_r2`: members with low
squared correlation to their cluster centroid are reassigned to cluster `0`
(the “other” cluster).

```{r prepare clustering arguments, eval = TRUE}
...
Note: Category 2 and 3 hits are only evaluated in integrated mode (and only if
their respective thresholds are > 0).
...
`create_clustering_report()` consumes the precomputed `report_payload`
returned by `cluster_hits()` and generates an HTML report with the
...
Depending on the settings used in `cluster_hits()`, the report may also
reflect effect-size filtering (`min_effect_size`) and optional pruning of
clusters via `min_cluster_r2`, where low-centroid-similarity features are
assigned to cluster `0` (“other”). If `raw_data` is provided, imputed
values can be highlighted in the spline plots.
...
```{r run ORA bioconductor, eval = FALSE}
ora_results <- SplineOmics::run_ora(
    cluster_table = clustering_results[["cluster_table"]],
    databases = databases,
    clusterProfiler_params = clusterProfiler_params,
...
```{r create ORA HTML report bioconductor, eval = FALSE}
plots <- SplineOmics::create_ora_report(
    report_payload = ora_results$report_payload,
    report_info = report_info,
    cluster_hits_report_name = "get_started",
