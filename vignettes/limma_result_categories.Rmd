...
limma analysis results can be divided into three categories, which are defined
in this document:
...
2.  **Average Difference Between Conditions**: This category compares the
average feature values for levels within a condition, regardless of time.

3.  **Interaction Between Condition and Time**: This category examines the
interaction between time and condition. It identifies features whose value
changes differently over time depending on the condition.
...
For example, bioreactor phase is a condition, and exponential and stationary 
are levels within that condition.
...
treatment, the feature must have different temporal patterns
in both conditions/levels) → Hit
...
# Second downward trend with a slightly different slope
y2 <- 0.65 - 0.4 * (x / max(x)) 
