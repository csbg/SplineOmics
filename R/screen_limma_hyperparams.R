...
plot_limma_combos_results <- function(
  ...
  message("Generating the plots for all pairwise hyperparams-combo comparisons")
...
generate_reports <- function(
    ...
    timestamp) {
  message(
    "Building .html reports for all pairwise hyperparams-combo comparisons"
  )
...
hc_vennheatmap <- function(hc_obj) {
  ...
  no_hits_1 <- all(vapply(hits_1, function(x) length(x) == 0, logical(1)))
  no_hits_2 <- all(vapply(hits_2, function(x) length(x) == 0, logical(1)))
...
gen_hitcomp_plots <- function(combo_pair) {
  ...
  combo_names <- vapply(names(combo_pair_combined), function(name) {
  ...
  if (length(plot_list) > 0) {
    ...
    composite_plot <- patchwork::wrap_plots(plot_list, ncol = 3) +
      patchwork::plot_annotation(
        title = paste(top_table_name, type,
          sep = " | "
        ),
